...
@IonicPage()
@Component({
  selector: 'page-quiz-solarsystem',
  templateUrl: 'quiz-solarsystem.html',
})
export class QuizSolarsystemPage {
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, public formBuilder: FormBuilder, public alertCtrl: AlertController, public dataService: DataSolarsystemProvider, public afAuth: AngularFireAuth, public af: AngularFireDatabase) {
  ...
  continue() {
    ...
    if (this.userProgressIDArr.length == 2) {
      //Edit Here
      if (this.score >= 7) {
        this.quizID = this.quizAstronomy.push(
          {
            Chapter_Quiz: "SolarSystem", Passed: true, Score: this.score, Quiz: 3
          }
        ).key;
        ...
        this.quizID = this.quizAstronomy.push(
          {
            Chapter_Quiz: "SolarSystem", Passed: false, Score: this.score, Quiz: 3
          }
        ).key;
        console.log(this.quizID);
        this.progressID.push({
      ...
      this.userProgressID.subscribe(snapshots => {
        ...
        console.log(this.userProgressKey[2]);
        console.log(this.userProgressIDArr[2].QuizID);
        ...
        this.userQuizID.subscribe(snapshots => {
          ...
          console.log(this.userQuizIDArr[2]);

          for (var i = 0; i <= this.userQuizIDArr.length; i++) {
            //Add for loop for the ProgressKey
            if (this.userProgressIDArr[2].QuizID === this.userQuizIDArr[i]) {
              this.updaterUPID = this.af.list('/UserProgress/' + this.currentUser + '/');
              this.updaterUPID.update(this.userProgressKey[2], { Score: this.score });
